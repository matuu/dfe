...
""" Modificaciones realizadas por Matias Varela <matu.varela@gmail.com> """
...
AUTHFILE = os.path.abspath(os.path.join(BASE_DIR, 'certs'))

WSDL = "https://wswhomo.afip.gov.ar/wsfev1/service.asmx?WSDL"
#CUIT = 30714274976  # ticketek 33690765549
CUIT = 20319419234
CERT = os.path.join(AUTHFILE, 'apim.crt')
PRIVATEKEY = os.path.join(AUTHFILE, 'apim.key')
CACERT = os.path.join(AUTHFILE, 'afip_ca_info.crt')
CACHE = os.path.join(AUTHFILE, 'cache')

# Autenticación:
wsaa = WSAA()
tra = wsaa.CreateTRA(service="wsfe")
cms = wsaa.SignTRA(tra, CERT, PRIVATEKEY)
wsaa.Conectar(cache=CACHE)
wsaa.LoginCMS(cms)
...
class TestFE(TestCase):
    ...
    def atest_dummy(self):
        print self.wsfev1.client.help("dummy")
        self.wsfev1.Dummy()
        print "AppServerStatus", self.wsfev1.AppServerStatus
        print "DbServerStatus", self.wsfev1.DbServerStatus
        print "AuthServerStatus", self.wsfev1.AuthServerStatus
    ...
    def test_autorizar_comprobante(self, tipo_cbte=1, cbte_nro=None, servicios=True):
        ...
        if not cbte_nro:
            ...
            print "Comprobante nro:", cbte_nro
